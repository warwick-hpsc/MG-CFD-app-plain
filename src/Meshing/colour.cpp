	...
	int* number_of_colours)
{
	...
	*edge_colours = alloc<int>(nedges);
	...
	for (long i=0; i<nedges; i++) (*edge_colours)[i] = UNCOLOURED;
	bool uncoloured_edges_remain=true;

	int* coloured_neighbour_counts = alloc<int>(nedges);
	for (long i=0; i<nedges; i++) coloured_neighbour_counts[i] = 0;
	...
	long nedges_coloured=0;
	long search_start=0;
	while(uncoloured_edges_remain) {
		while (!edge_queue.empty()) {
			long eid = edge_queue.top().eidx;
			edge_queue.pop();
		...
		for (long e=search_start; e<num_internal_edges; e++) {
		...
		if (!uncoloured_edges_remain) {
			...
			for (long e=search_start; e<boundary_edges_start+num_boundary_edges; e++) {
				if ((*edge_colours)[e] == UNCOLOURED) {
					uncoloured_edges_remain = true;
					edge_record er;
					er.eidx = e;
		...
		if (!uncoloured_edges_remain) {
			...
			for (long e=search_start; e<wall_edges_start+num_wall_edges; e++) {
				if ((*edge_colours)[e] == UNCOLOURED) {
					uncoloured_edges_remain = true;
					edge_record er;
					er.eidx = e;
	...
	// Check all edges have been coloured
	#ifdef TESTS
		for (long e=0; e<num_internal_edges; e++) {
			if ((*edge_colours)[e] == UNCOLOURED) {
				printf("Edge %d not coloured. a=%d, b=%d\n", e, edges[e].a, edges[e].b);
				// Does edge have neighbours?
				for (long en=0; en<nedges; en++) {
					long a=edges[e].a, b=edges[e].b, an=edges[en].a, bn=edges[en].b;
		...
		for (long e=boundary_edges_start; e<boundary_edges_start+num_boundary_edges; e++) {
			if ((*edge_colours)[e] == UNCOLOURED) {
				printf("Edge %d not coloured. a=%d, b=%d\n", e, edges[e].a, edges[e].b);
				// Does edge have neighbours?
				for (long en=0; en<nedges; en++) {
		...
		for (long e=wall_edges_start; e<wall_edges_start+num_wall_edges; e++) {
			if ((*edge_colours)[e] == UNCOLOURED) {
				printf("Edge %d not coloured. a=%d, b=%d\n", e, edges[e].a, edges[e].b);
				// Does edge have neighbours?
				for (long en=0; en<nedges; en++) {
	...
	*number_of_colours = max_edge_degree;
	...
	for (long p=0; p<npoints; p++) {
		if (point_degree[p] > 0)
			dealloc<long>(point_to_edges[p]);
	}
	dealloc<long*>(point_to_edges);
	dealloc<int>(point_degree);
	for (long e=0; e<nedges; e++) {
		if (edge_degree[e] > 0)
			dealloc<long>(edge_neighbours[e]);
	}
	dealloc<long*>(edge_neighbours);
