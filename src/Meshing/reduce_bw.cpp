...
int* reduce_bw_of_points(
	...
	int* mg_connectivity_level_below, 
	int mg_connectivity_level_below_size, 
	bool reverse) 
{
	int* point_degree = 
	...
	int lowest_degree_point = 0;
	int lowest_degree = point_degree[0];
	...
	int* point_queue = alloc<int>(npoints);
	for (int i=0; i<npoints; i++) point_queue[i] = -1;
	bool* queued_points = alloc<bool>(npoints);
	for (int i=0; i<npoints; i++) queued_points[i] = false;
	int perm_write = 0;
	point_queue[0] = lowest_degree_point;
	queued_points[lowest_degree_point] = true;
	int queue_read=0, queue_write=1;
	...
	while (queue_read < npoints) {
		...
		for (int n=0; n<point_degree[p]; n++) {
		...
		if (queue_read == queue_write && queue_read < npoints) {
			...
			for (int p=search_start; p<npoints; p++) {
